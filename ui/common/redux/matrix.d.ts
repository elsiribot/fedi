...
import { MatrixUser, MatrixRoom, MatrixAuth, MatrixRoomMember, MatrixError, MatrixPowerLevel, MatrixSearchResults, MatrixPaymentEvent, MatrixRoomListObservableUpdates, MatrixTimelineObservableUpdates, MatrixRoomListItem, MatrixTimelineItem, MatrixEvent, MatrixRoomPowerLevels, MatrixSyncStatus, Sats, MatrixGroupPreview, FederationListItem } from '../types';
...
declare const initialState: {
    ...
    groupPreviews: Record<string, MatrixGroupPreview>;
};
...
export declare const previewDefaultGroupChats: import("@reduxjs/toolkit").AsyncThunk<MatrixGroupPreview[], void, {
    state: CommonState;
    dispatch?: import("redux").Dispatch<import("redux").AnyAction> | undefined;
    extra?: unknown;
    rejectValue?: unknown;
...
export declare const selectCanPayFromOtherFeds: ((state: CommonState, chatPayment: MatrixPaymentEvent) => boolean) & import("reselect").OutputSelectorFields<(args_0: FederationListItem[], args_1: MatrixPaymentEvent) => boolean & {
    clearCache: () => void;
}> & {
    clearCache: () => void;
};
export declare const selectCanSendPayment: ((state: CommonState, chatPayment: MatrixPaymentEvent) => boolean) & import("reselect").OutputSelectorFields<(args_0: FederationListItem[], args_1: MatrixPaymentEvent) => boolean & {
    clearCache: () => void;
}> & {
    clearCache: () => void;
};
export declare const selectCanClaimPayment: ((state: CommonState, chatPayment: MatrixPaymentEvent) => boolean) & import("reselect").OutputSelectorFields<(args_0: FederationListItem[], args_1: MatrixPaymentEvent) => boolean & {
    clearCache: () => void;
}> & {
    clearCache: () => void;
};
export declare const selectCommunityDefaultRoomIds: ((state: CommonState, federationId: string) => string[]) & import("reselect").OutputSelectorFields<(args_0: FederationListItem | undefined) => string[] & {
    clearCache: () => void;
}> & {
    clearCache: () => void;
};
export declare const selectDefaultMatrixRoomIds: ((state: CommonState) => string[]) & import("reselect").OutputSelectorFields<(args_0: FederationListItem[], args_1: import("../types").ClientConfigMetadata | undefined) => string[] & {
    clearCache: () => void;
}> & {
    clearCache: () => void;
};
