...
import type { JSONObject, ObservableVecUpdate, RpcMatrixMembership } from './bindings';
export { MatrixEventContent };
export declare enum MatrixSyncStatus {
...
export declare enum MatrixRoomListItemStatus {
    loading = "loading",
    ready = "ready"
}
...
export type MatrixRoomListObservableUpdates = ObservableVecUpdate<MatrixRoomListItem>['update'];
...
export declare enum MatrixEventStatus {
    pending = "pending",
    sent = "sent",
    failed = "failed",
    cancelled = "cancelled"
}
export interface MatrixEvent<Content extends MatrixEventContent = MatrixEventContent> {
    id: string;
    content: Content;
...
export type MatrixTimelineObservableUpdates = ObservableVecUpdate<MatrixTimelineItem>['update'];
export type MatrixError = Error;
export declare enum MatrixPowerLevel {
    Member = 0,
    Moderator = 50,
    Admin = 100
}
...
export declare enum MatrixPaymentStatus {
...
type PickEventContentType<T, K extends MatrixEventContent['msgtype']> = T extends {
    msgtype: K;
} ? T : never;
export type MatrixPaymentEventContent = PickEventContentType<MatrixEventContent, 'xyz.fedi.payment'>;
